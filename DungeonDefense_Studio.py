...
COLOR_ENEMY_SCOUT = (220, 120, 0)  # New Enemy Color
COLOR_ENEMY_BOSS = (255, 255, 0)  # Boss Color
...
COLOR_GOLD_MINE = (255, 223, 0)  # Gold Mine Color
    ...
    def __init__(self):
        self.data = {
            ...
            'upgrades': {'spike_damage': 0, 'spike_health': 0, 'slow_duration': 0, 'slow_health': 0, 'turret_damage': 0,
                         'turret_range': 0, 'gold_income': 0},  # Added gold_income
            'rank': 'Captain'
        }
        self.load()
    ...
    def load(self):
        if os.path.exists(STATS_FILE):
            try:
                ...
                # --- Gracefully add new keys if they don't exist ---
...
class Enemy(pygame.sprite.Sprite):
    ...
    def check_death(self):
        if self.health <= 0:
            ...
            self.game.register_kill(self.value)
            self.kill()
    ...
    def draw(self, surface):
        ...
        if self.is_slowed:
            pygame.draw.circle(surface, COLOR_SLOW_TRAP_ACTIVE, self.rect.center, self.rect.width // 2, 2)
...
class Boss(Enemy):  # NEW BOSS ENEMY
    def __init__(self, path, health, game):
...
class TurretTrap(Trap):
    ...
    def draw(self):
        ...
        end_x = 16 + 16 * math.cos(self.angle)
        end_y = 16 + 16 * math.sin(self.angle)
    ...
    def update(self, dt, game):
        ...
        if self.target:
            ...
            if self.timer <= 0:
                ...
                if self.is_ultimate:  # Fires two projectiles
...
class GoldMine(Trap):  # NEW TRAP
    def __init__(self, x, y, research):
        super().__init__(x, y, 60, 40, 50, research)
        rank_bonus = self.research.get_rank_bonus()
...
class Game:
    ...
    def setup_research_buttons(self):
        ...
        y_start, x_start = 150, SCREEN_WIDTH // 2 + 250
        upgrades = {
            'spike_damage': 'Spike Dmg', 'spike_health': 'Spike HP',
            'slow_duration': 'Slow Time', 'slow_health': 'Slow HP',
            'turret_damage': 'Turret Dmg', 'turret_range': 'Turret Rng',
            ...
            if self.game_state == "playing":
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key in key_map:
                        ...
                        if self.money >= self.selected_trap_instance.upgrade_cost and not self.selected_trap_instance.is_ultimate:
    ...
    def sell_trap(self, trap):
        refund = int(trap.total_investment * 0.7)  # Sell for 70% of total cost
        self.money += refund
    ...
    def place_trap(self, x, y):
        ...
        if self.money >= cost:
            ...
            trap_class = trap_classes[self.selected_trap_type]
    ...
    def update(self, dt):
        ...
        for enemy in list(self.enemies):
            if enemy.path_index >= len(self.path_list) - 1:
                self.lives -= 1 if not isinstance(enemy, Boss) else 10  # Boss deals more damage
                enemy.kill()
    ...
    def start_wave(self):
        ...
        if self.wave % BOSS_WAVE_INTERVAL == 0:
            boss_health = base_health * 15 * (1 + self.wave // BOSS_WAVE_INTERVAL)
            self.enemies.add(Boss(self.path_list, boss_health, self))
            return
        ...
        for i in range(self.wave * 4 + 5):
            enemy_class = random.choice(enemy_pool)
            enemy = enemy_class(self.path_list, base_health, self)
            spawn_offset = i * (GRID_SIZE * 1.5)  # Space enemies out more
            enemy.rect.centerx -= spawn_offset
    ...
    def draw_ui(self):
        ...
        if not self.wave_in_progress and self.wave < TOTAL_WAVES:
            prompt = self.small_font.render("SPACE for next wave (+$25)", True, COLOR_TEXT)
            self.screen.blit(prompt, (180, SCREEN_HEIGHT - 55))

        # Shockwave Ability UI
