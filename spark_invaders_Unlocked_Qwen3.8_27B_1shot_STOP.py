...
"""
SPACE INVADERS — fully procedural version (pygame)
===================================================
Run:      python space_invaders.py        (needs: pip install pygame)
Controls: < / > or A,D  move   |  SPACE shoot (hold)
          P pause      |  M mute   |  ESC menu / quit

All graphics (pixel sprites, glow, bunkers, stars, nebulae) and all audio
(sound effects + looping chiptune music) are generated at runtime in Python.

External files (created automatically in ./data next to this script):
  config.json   - every gameplay parameter (edit while game is closed!)
  stats.json    - lifetime statistics (kills, accuracy, playtime, ...)
  progress.json - high score, best wave, last game info
"""
...
class _NullSound:
    ...
    def set_length(self, n): pass

def _loop_forever(sound):
    """Make a Sound loop forever; works on pygame 1.x and 2.x."""
    if hasattr(sound, "set_length"):
        sound.set_length(-1)
    else:
        sound.set_num_repeats(-1)
...
class Game:
    ...
    def __init__(self):
        ...
        # load save files first, then build the sound bank exactly once
