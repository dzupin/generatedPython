...
class PowerUp:
    ...
    TYPES = POWER_UP_TYPES  # Reference the module-level dictionary
...
class Achievement:
    ...
    def __init__(self):
        ...
        self.PowerUp = PowerUp  # Add this line
...
class Player:
    def __init__(self):
        ...
        self.POWER_UP_TYPES = POWER_UP_TYPES  # Reference the module-level dictionary
    ...
    def activate_power(self, power_type):
        if power_type == 'rapid':
            ...
            self.power_timer = self.POWER_UP_TYPES['rapid']['duration']
        elif power_type == 'extra_life':
            self.lives += 1
            return True
        elif power_type == 'shield':
            self.shield_active = 300  # 5 seconds at 60 FPS
            self.active_power = 'shield'
            self.power_timer = self.POWER_UP_TYPES['shield']['duration']
        elif power_type == 'slow':
            self.active_power = 'slow'
            self.power_timer = self.POWER_UP_TYPES['slow']['duration']
        return False
...
class Alien:
    ...
    def update(self, dx, dy, alien_count):
        self.x += dx
        self.y += dy
...
class AlienGrid:
    def __init__(self):
        ...
        self.base_speed = 1
        self.level = 1  # Initialize level here
        self.create_grid()  # Now level is available
    ...
    def create_grid(self):
        ...
        for row in range(rows):
            for col in range(cols):
                alien_type = min(row // 2, 2)  # Different types in different rows
    ...
    def next_level(self):
        self.level += 1
        self.base_speed = min(1 + self.level * 0.2, 4)  # Cap speed at 4
        self.create_grid()
    ...
    def update(self):
        ...
        # Pass the alien count to each alien's update method
        alien_count = len(self.aliens)
        for alien in self.aliens:
            alien.update(dx, dy, alien_count)
...
class Barrier:
    ...
    def draw(self):
        for block in self.blocks[:]:
            ...
            health_ratio = len(self.blocks) / 60  # Approximate initial block count
    ...
    for bullet in player.bullets[:]:
        ...
        for alien in alien_grid.aliens[:]:
            if bullet_rect.colliderect(alien.get_rect()):
                ...
                player.combo_timer = 120  # 2 seconds to maintain combo
...
def main():
    ...
    game_state = "start"  # start, playing, game_over, level_complete
    game_over = False
    game_won = False
    level = 1
    game_start_time = 0
