...
# --- NEW: Save File and Upgrade Management ---
player_upgrades = {}
...
def load_upgrades():
    ...
    if os.path.exists(SAVE_FILE):
        ...
        # Default structure if no save file exists
        player_upgrades = {
            "research_points": 0,
            "upgrades": {
                "starting_money": 0,
...
game_state = "MAIN_MENU"  # NEW: Manages the current screen
game = None  # Will hold the main Game object when active
research_menu = None  # Will hold the menu object
...
class Enemy:
    ...
    def update(self):
        ...
        if norm > 0: self.pos[0] += direction_x / norm * current_speed; self.pos[
            1] += direction_y / norm * current_speed
    ...
    def draw(self, surface):
        ...
        health_bar_width, health_bar_height, health_ratio = self.size, 5, self.health / self.max_health
...
class Tower:
    # --- UPDATED: Tower stats now affected by research upgrades ---
    ...
    def set_stats(self):
        if self.type == "gatling":
            ...
            base_damage = [8, 15, 25][self.level - 1]
            damage_bonus = 1 + (player_upgrades["upgrades"]["gatling_damage"] * 0.05)  # 5% per level
            self.damage = int(base_damage * damage_bonus)
            self.range, self.cooldown = [120, 140, 160][self.level - 1], [20, 15, 10][self.level - 1]
            ...
            base_cost = 250
            cost_reduction = 1 - (player_upgrades["upgrades"]["cannon_cost"] * 0.03)  # 3% per level
    ...
    def upgrade(self):
        if self.level < 3 and game.player_money >= self.upgrade_cost:
            game.player_money -= self.upgrade_cost
            self.level += 1
            self.set_stats()

    # ... (Rest of Tower class is unchanged) ...
    ...
    def draw(self, surface):
        if game.selected_tower is self:
...
class Projectile:
    # ... (No changes to Projectile class) ...
...
class Game:
    # --- UPDATED: Game now initializes with stats from player_upgrades ---
    def __init__(self):
    ...
    def start_next_wave(self):
        if not self.wave_spawn_list and self.current_wave < len(wave_definitions):
            if self.wave_cooldown < self.time_between_waves:
                self.player_money += int(((self.time_between_waves - self.wave_cooldown) / self.time_between_waves) * (
    ...
    # ... (Rest of Game class is mostly the same, but uses self.player_money etc.) ...
    def handle_events(self, events):
        for event in events:
            if event.type == pygame.QUIT: global game_running; game_running = False
    ...
    def place_tower(self, pos):
        ...
        if self.player_money >= new_tower.cost:
    ...
    def update(self):
        global game_state
        if self.player_health <= 0: game_state = "GAME_OVER"; return
    ...
    def draw_ui(self, surface):
        ...
        surface.blit(FONT_UI.render(f"Health: {self.player_health}", True, COLOR_TEXT), (panel_rect[0] + 20, 20))
        surface.blit(FONT_UI.render(f"Money: ${self.player_money}", True, COLOR_TEXT), (panel_rect[0] + 20, 50))
        ...
        if self.selected_tower:
            ...
            surface.blit(FONT_TITLE.render(f"Lvl {st.level} {st.type.capitalize()}", True, COLOR_TEXT),
                         (panel_rect[0] + 20, 380))
            surface.blit(FONT_UI.render(f"Damage: {st.damage}", True, COLOR_TEXT), (panel_rect[0] + 20, 420))
            surface.blit(FONT_UI.render(f"Range: {st.range}", True, COLOR_TEXT), (panel_rect[0] + 20, 445))
            if st.type != "slowing": surface.blit(
                FONT_UI.render(f"Speed: {round(60 / st.cooldown, 1)}/s", True, COLOR_TEXT), (panel_rect[0] + 20, 470))
        ...
        if not self.wave_spawn_list and not self.enemies and self.current_wave < len(wave_definitions):
            ...
            if bonus > 0: surface.blit(FONT_UI.render(f"Rush Bonus: ${bonus}", True, (255, 215, 0)),
                                       (panel_rect[0] + 20, SCREEN_HEIGHT - 110))


# --- NEW: Research Menu Class ---
class ResearchMenu:
    def __init__(self):
        self.buttons = []
        self.setup_menu()
    ...
    def draw(self, surface):
        ...
        # RP Display
        rp_surf = FONT_UI.render(f"Research Points Available: {player_upgrades['research_points']} RP", True,
                                 (255, 215, 0))
    ...
    def draw_upgrade_info(self, surface, name, title, effect_text, x, y):
        ...
        title_surf = FONT_UI.render(f"{title} (Lvl {level})", True, COLOR_TEXT)
        effect_surf = FONT_UI.render(f"Current: {effect_text}", True, (150, 255, 150))
        cost_surf = FONT_UI.render(f"Cost: {cost} RP", True, (255, 215, 0))

        surface.blit(title_surf, (x, y - 40))
        surface.blit(effect_surf, (x, y))
        surface.blit(cost_surf, (x, y + 25))
...
if __name__ == "__main__":
    ...
    while game_running:
        ...
        if game_state == "MAIN_MENU":
            ...
            # This block runs only ONCE when the state changes
            if game:
                earned_rp = game.current_wave * 10
