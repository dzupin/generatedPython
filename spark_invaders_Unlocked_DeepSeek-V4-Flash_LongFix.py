...
class SoundBank:
    ...
    def play_music(self):
        if self.ok and not self.muted:
            self.music.play(-1)   # -1 = infinite loop
...
# ----------------------------------------------------------------------------
class Bunker:
    CW, CH = 12, 8   # much smaller barriers (was 22, 16)
    ...
    def _shape(self, x, y):
        if x < 0 or y < 0 or x >= self.CW or y >= self.CH:
            return False
        # central vertical channel (x=5..6) lets player bullets pass through
        if 5 <= x < 7:
            return False
        return True
...
class Formation:
    ...
    def update(self, dt, g):
        ...
        mx = 130        # was 18 — wide sweep so enemies come out from behind barriers
        if self.xoff > mx:
            self.xoff = mx
            self.dx = -1.0
            self._step_down()
    ...
    def _step_down(self):
        self.oy += 18   # was 26 — slower descent
...
class Game:
        ...
        if self.respawn_t > 0:
            ...
            if want and self.fire_cd <= 0:
                ...
                if "double" in self.effects:
                    for off in (-6, 6):
                        self.bullets.append(Bullet(self.player.centerx + off, top, 0, -bspd, "player"))
                    rs["shots"] += 2
                else:
                    self.bullets.append(Bullet(self.player.centerx, top, 0, -bspd, "player"))
                    rs["shots"] += 1
    ...
    def draw(self):
        ...
        # draw particles/texts onto the world surface, then blit ONCE
